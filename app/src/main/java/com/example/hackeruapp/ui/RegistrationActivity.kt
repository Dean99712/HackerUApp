...
import com.example.hackeruapp.LoginFragment
import com.example.hackeruapp.MainActivity
import com.example.hackeruapp.R
import com.example.hackeruapp.SignupFragment
...
class RegistrationActivity : AppCompatActivity() {
    ...
    private fun calculateLastLogin() {
        ...
        if (lastLogin != -1L && System.currentTimeMillis() - lastLogin < 86400000) {
    ...
    fun displayLoginFragment() {
        ...
        supportFragmentManager.beginTransaction()
                .replace(R.id.fragment_registration, loginFragment)
                .commit()
    }
    ...
    private fun isUserLegit(): Boolean {
        return findViewById<EditText>(R.id.email_login_tv).text.toString() == userName &&
                findViewById<EditText>(R.id.password_register_et).text.toString() == password
    }
}